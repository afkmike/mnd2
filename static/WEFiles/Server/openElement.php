...
class OEUploadFile {
	...
	var $DestFileName; //Nom du fichier de destination final
	...
	//$this->DestFileName : nom du fichier final
	//Renvoi : OEReturn (error ou success)
	 function GetFile($FiedName, $DestFileName,$WEInfoPage) {

		//if (!isset ($_FILES[$FiedName])) { return new OEReturn ("error","Erreur de téléchargement - Fichier introuvable.");}

		$FileNameTmp=$_FILES[$FiedName]['tmp_name'];
		$FileName=$_FILES[$FiedName]['name'];
		
		if (empty($FileName)) { return new OEReturn ("success","");} //champ vide
		...
		if ($_FILES[$FiedName]["size"] == 0) {exit(0);}
		...
		if ($this->allowedMaxSize>0) {
			if ($_FILES[$FiedName]["size"] > $this->allowedMaxSize) { $ErrorDescription="Taille du fichier trop grande.";}//GetErrMessage($objJson,$WEInfoPage,"ErrorSize");} //"Taille du fichier incorrecte."
		}
		...
		if (empty($DestFileName)) {
			...
			$DestFileName = $this->dir_extract."/".$DestFileName;
		}

		$this->DestFileName=$DestFileName;

		if (!move_uploaded_file($FileNameTmp, $this->DestFileName)) 
		{
