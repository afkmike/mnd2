...
//Recup de la valeur
	...
	if (isset($param[$FormLinksElement->ID])) {
		$val=$param[$FormLinksElement->ID];
		$val = htmlentities($val, $htmlEntFlags, 'UTF-8'); // ENT_SUBSTITUTE is only from PHP 5.4
	} else {
		//ElementForm Version2
		if (isset($ElemConf->$FormLinksElemenID->InputName) && isset($param[$ElemConf->$FormLinksElemenID->InputName])) { // DD ex checkboxes may write nothing at all if unchecked => absent in $param
			$val=$param[$ElemConf->$FormLinksElemenID->InputName];
			$val = htmlentities($val, $htmlEntFlags, 'UTF-8'); // ENT_SUBSTITUTE is only from PHP 5.4
		} else {
			$val="-";
		}
	...
	switch($ElemType) {
		...
		case 'WEUploadFiles';
			...
			$rep=$Upload->GetFile($uploadFieldName,"",$WEInfoPage);
			if ($rep->State=="error") {
			...
			if (!empty($Upload->DestFileName)) array_push($Attachment,$Upload->DestFileName);
